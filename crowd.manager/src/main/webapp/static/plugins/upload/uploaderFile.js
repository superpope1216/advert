...
var Uploader = (function($) {
	...
	var uploader = function(opt) {
		this.params = {
			...
			title: "",  // 文件名
			readOnly: false,	// 是否只读
			rootPath: basePath,	// URL根路径
			uploadUrl: "/attr/upload.do",	// 上传的URL
			downloadUrl: "/attr/download.do", // 下载的URL
			removeUrl: "/attr/remove.do", // 删除的URL
			fileViewNameUrl: "/attr/getFileViewName.do", // 获取文件显示名称的URL
		...
		checkSize: function(id) {
			...
			try {
				...
				if (filesize == -1) {
					...
					alert("上传文件不能超过10M");
				//    console.log("maxSize"+this.params.maxSize);
					return false;
				}
			} catch(e) {
				console.log(e);
