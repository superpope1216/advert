...
$(document).ready(function() {
	...
	var tbl=createMyTable("#example",basePath+"/sjglDlxx/getList",queryParam,tblColums);
	...
	$("#example").on("click","[action='detailRow']",function(){
		var key=$(this).attr("data-key");
		doGet(basePath+"/sjglDlxx/select","wid="+key,function(data){
	...
	$("#example").on("click","[action='delRow']",function(){
		...
		confirm("确认删除吗？",function(){
			doPost(basePath+"/sjglDlxx/delete","wid="+key,function(){
				alert("删除成功！");
				tbl.draw();
			});
		});
	...
	$("#btnSh").click(function(){
		confirm("确认保存吗？",function(){
			doPost(basePath+"/sjglDlxx/sh",$("#shForm").serializeArray(),function(){
				$("#modal-sh").modal("hide");
				tbl.draw(false);
			});
		});
