...
define(function(require, exports, module) {
	...
	$("#btnSupply").click(function(){
		window.location.href=basePath+"/dlglEdit/index";
	});
	...
	function getPesronDllb(pageNum,needPage){
		...
		doGet(basePath+"/dlglView/queryPersonList","pageNum="+pageNum,function(data){
			if(data && data.datas){
				if(needPage){
					$('#pageData').pagination({
				...
				$("#tblData button").click(function(){
					...
					var _key=$(this).attr("data-key");
					if(_option=="view"){
						window.open(basePath+"/dlglView/view?wid="+_key);
					}else if(_option=="edit"){
						window.location.href=basePath+"/dlglEdit/index?wid="+_key;
					}else if(_option=="delete"){
						confirm("您确认删除该信息吗？",function(){
							doPost(basePath+"/dlglView/delete","wid="+_key,function(){
								alert("删除成功！");
								getPesronDllb(1,true);
							});
						})
	...
	function getDllb(pageNum,needPage){
		...
		doGet(basePath+"/dlglView/queryAllList","pageNum="+pageNum,function(data){
			if(data && data.datas){
				if(needPage){
