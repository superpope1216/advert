...
define(function(require, exports, module) {
	...
	$("#btnSupply").click(function(){
		window.location.href=basePath+"/zbglEdit/index";
	});
	...
	function getPesronDllb(pageNum,needPage){
		...
		doGet(basePath+"/zbglView/queryPersonList","pageNum="+pageNum,function(data){
			if(data && data.datas){
				if(needPage){
					$('#pageData').pagination({
				...
				$("#tblData button").click(function(){
					...
					var _key=$(this).attr("data-key");
					if(_option=="view"){
						window.open(basePath+"/zbglView/view?wid="+_key);
					}else if(_option=="edit"){
						window.location.href=basePath+"/zbglEdit/index?wid="+_key;
					}else if(_option=="delete"){
						confirm("您确认删除该信息吗？",function(){
							doPost(basePath+"/zbglView/delete","wid="+_key,function(){
								alert("删除成功！");
								getPesronDllb(1,true);
							});
						})
	...
	function getDllb(pageNum,needPage){
		...
		doGet(basePath+"/zbglView/queryAllList","pageNum="+pageNum,function(data){
			if(data && data.datas){
				if(needPage){
				...
				for(var i=0;i<data.datas.length;i++){
					...
					html+='				<a href="'+basePath+'/zbglView/view?wid='+_data.wid+'" target="_blank" style="display:block;">';
					...
					html+='						<a href="'+basePath+'/zbglView/view?wid='+_data.wid+'" target="_blank"	class="info-body-title-name">';
